...
pdflatex informe_taller_3.tex
echo "📚 Procesando referencias bibliográficas..."
bibtex informe_taller_3
pdflatex informe_taller_3.tex
pdflatex informe_taller_3.tex
...
if [ -f informe_taller_3.pdf ]; then
    echo "📄 Documento generado:"
    ls -lh informe_taller_3.pdf
    echo "📊 Número de páginas: $(pdfinfo informe_taller_3.pdf 2>/dev/null | grep Pages | awk '{print $2}')"
fi

# Abrir el PDF si el sistema lo permite
if command -v xdg-open &> /dev/null; then
    echo "📖 Abriendo PDF..."
    xdg-open informe_taller_3.pdf &
elif command -v open &> /dev/null; then
    echo "📖 Abriendo PDF..."
    open informe_taller_3.pdf &
fi